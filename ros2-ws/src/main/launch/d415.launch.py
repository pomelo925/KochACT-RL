...
def launch_setup(context, params, param_name_suffix=''):
    _config_file = LaunchCnfiguration('config_file' + param_name_suffix).perform(context)
    ...
    nodes = []
    for camera in params_from_file.get('cameras', []):
        if camera.get('enabled', False):
            camera_params = params.copy()
            ...
            nodes.append(
                launch_ros.actions.Node(
                    ...
                    parameters=[camera_params, params_from_file.get('shared_settings', {})],
                    output=_output,
